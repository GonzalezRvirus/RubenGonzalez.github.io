...
<style> /* Modern responsive grid: auto-fit columns and a visually appealing gap */ .gallery-container { display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 20px; padding: 20px; max-width: 1200px; margin: 0 auto; /* Center the gallery on the page */ } /* Make each gallery item behave like a “card” */ .gallery-item { display: flex; flex-direction: column; align-items: center; text-align: center; background-color: #fff; /* Card background */ border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1); overflow: hidden; /* Ensure rounded corners crop images */ transition: transform 0.2s ease, box-shadow 0.2s ease; } /* Subtle hover effect for a modern look */ .gallery-item:hover { transform: translateY(-4px); box-shadow: 0 8px 20px rgba(0,0,0,0.1); } /* Container for the image so we can flex-center it */ .gallery-image { width: 100%; display: flex; justify-content: center; align-items: center; overflow: hidden; } /* Make the image nicely fit while preserving aspect ratio */ .gallery-image img { width: 100%; height: auto; object-fit: cover; /* or “contain” if you don't want to crop */ } /* Style the caption under each image */ .gallery-item p { margin: 10px 0; color: #666; font-size: 14px; font-family: Arial, sans-serif; padding: 0 10px 10px; } </style>
